...
const newsletterSchema = z.string().trim().email("Please enter a valid email address.").max(255, "Email must be 255 characters or fewer.");
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input id="name" value={form.name} onChange={(e) => { setForm({ ...form, name: e.target.value }); setFormErrors({ ...formErrors, name: undefined }); }} placeholder="Your name" required maxLength={100} aria-invalid={Boolean(formErrors.name)} className="rounded-xl" />
                  ...
                  <div>
                    ...
                    <Input id="email" type="email" value={form.email} onChange={(e) => { setForm({ ...form, email: e.target.value }); setFormErrors({ ...formErrors, email: undefined }); }} placeholder="you@example.com" required maxLength={255} aria-invalid={Boolean(formErrors.email)} className="rounded-xl" />
                ...
                <div>
                  ...
                  <Textarea id="message" value={form.message} onChange={(e) => { setForm({ ...form, message: e.target.value }); setFormErrors({ ...formErrors, message: undefined }); }} placeholder="How would you like to help?" rows={4} required maxLength={5000} aria-invalid={Boolean(formErrors.message)} className="rounded-xl" />
          ...
          <div className="reveal">
            ...
            <div className="p-8 rounded-2xl bg-card border shadow-sm mb-6">
              ...
              <form onSubmit={handleSubscribe} className="flex gap-2">
                <Input value={newsletter} onChange={(e) => { setNewsletter(e.target.value); setNewsletterError(""); }} placeholder="Your email" type="email" required maxLength={255} aria-invalid={Boolean(newsletterError)} className="rounded-xl flex-1" />
                <Button type="submit" variant="default" className="rounded-xl" disabled={subLoading}>
                  {subLoading ? "..." : "Subscribe"}
                </Button>
              </form>
              {newsletterError && <p className="mt-2 text-sm text-destructive">{newsletterError}</p>}
            </div>
